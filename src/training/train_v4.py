...
from utills.early_stopping import EarlyStopping
...
def val(net, dataset, criterion, optimizer, max_iter=parameters.val_iteration):
    ...
    for i in range(max_iter):
        ...
        # Wait for loading data for next iteration
    ...
    print(' Validation  loss------------: %f, accuracy-------------------: %f , Mean Edit Distance : %f' %
          (np.average(validation_losses), accuracy, mean_edit_distance))
    return np.average(validation_losses), accuracy
...
def main():
    ...
    for epoch in range(1, parameters.epochs):
        ...
        validation_details.write(
            f'Epoch ---- {epoch} cost --- {val_cost} accuracy --- {accuracy} \n')
        validation_cost[epoch] = val_cost
        validation_accuracy[epoch] = validation_accuracy

        print(f'Ending for epoch {epoch}')
        training_details.write(f'Epoch {epoch}     average_cost : {np.average(training_losses)} \n')
        training_cost[epoch] = np.average(training_losses)

        early_stopping(val_cost, crnn)

        if early_stopping.early_stop:
            print("Early stopping")
            break

        # torch.save(crnn.state_dict(), '{0}/test_early_stopping{1}.pth'.format(parameters.weights_folder, epoch))
